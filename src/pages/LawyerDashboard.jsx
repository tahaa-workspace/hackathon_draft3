...
import { getClaimFileUrl, getLawyerClaims, reviewClaimAsLawyer } from '../services/legacyService';
...
function RecordsGroup({ title, icon: Icon, records }) {
  return (
    <div>
      <div className="mb-2 flex items-center gap-2"><Icon size={15} className="text-brand-600" /><p className="text-xs font-semibold uppercase tracking-wide text-ink-500">{title} ({records.length})</p></div>
      {records.length === 0 ? <p className="rounded-xl bg-ink-50 px-3 py-3 text-xs text-ink-400">No {title.toLowerCase()} assigned.</p> : (
        <div className="space-y-2">{records.map((doc) => <div key={doc.id} className="rounded-xl border border-ink-100 px-3 py-3"><p className="text-sm font-semibold text-ink-800">{doc.title}</p><p className="text-xs text-ink-400">{doc.category} · {doc.originalName}</p></div>)}</div>
      )}
    </div>
  );
}
...
export default function LawyerDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try { setClaims(await getLawyerClaims()); }
    catch (err) { setError(err.message || 'Unable to load assigned Legacy Access Claims.'); }
 ...
 const openEvidence = async (claimId, kind) => {
  ...
  const review = async (claim, action) => {
    ...
    const confirmation = action !== 'REQUEST_MORE_INFORMATION' ? window.confirm(action === 'APPROVE' ? 'Complete professional review and release only the Owner-assigned Assets, Liabilities and General records to this Beneficiary?' : 'Place this Legacy Access Claim on hold? Assigned information will remain locked.') : true;
    if (!confirmation) return;
    setWorkingId(claim.id); setError('');
    try { await reviewClaimAsLawyer(claim.id, action, remarks); await load(); }
        ...
        {loading ? <div className="card flex items-center justify-center gap-2 py-16 text-sm text-ink-500"><Loader2 size={18} className="animate-spin" /> Loading assigned cases…</div> : visibleClaims.length === 0 ? <div className="card py-16 text-center"><Scale size={34} className="mx-auto text-ink-300" /><p className="mt-3 font-semibold text-ink-800">No cases in this section</p><p className="mt-1 text-sm text-ink-400">Only Admin-assigned Legacy Access Claims appear here.</p></div> : (
          <div className="space-y-5">{visibleClaims.map((claim) => {
            ...
            return <section key={claim.id} className="overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-sm">
              ...
              {['UNDER_LAWYER_REVIEW', 'MORE_INFORMATION_REQUIRED'].includes(claim.status) && <div className="flex flex-wrap justify-end gap-2 border-t border-ink-100 bg-ink-50/30 px-5 py-4"><button disabled={workingId === claim.id} onClick={() => review(claim, 'REQUEST_MORE_INFORMATION')} className="rounded-lg border border-orange-200 px-3 py-2 text-xs font-semibold text-orange-700 hover:bg-orange-50">Request More Information</button><button disabled={workingId === claim.id} onClick={() => review(claim, 'HOLD')} className="rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-600 hover:bg-red-50"><PauseCircle size={14} className="inline" /> Hold / Raise Concern</button><button disabled={workingId === claim.id} onClick={() => review(claim, 'APPROVE')} className="btn-primary"><CheckCircle2 size={15} /> Complete / Approve Review</button></div>}
