...
export default function AdminLegacyClaims() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setClaims(claimData); setLawyers(lawyerData);
    } catch (err) { setError(err.message || 'Unable to load Legacy Access Claims.'); }
  ...
  const review = async (claim, action) => {
    ...
    setWorkingId(claim.id); setError('');
    try { await reviewClaimAsAdmin(claim.id, action, remarks); await load(); }
    catch (err) { setError(err.message); }
  ...
  return (
    <div className="min-h-screen bg-ink-50">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-7"><p className="text-sm font-semibold text-brand-600">Administration</p><h1 className="mt-1 text-3xl font-bold tracking-tight text-ink-900">Legacy Access Claims</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-ink-500">Verify the pre-existing Owner-Beneficiary link, submitted evidence, and that Owner-assigned vault records exist. Admin approval confirms platform checks only; it does not decide inheritance rights.</p></div>
        {error && <div className="alert-error mb-5">{error}</div>}

        {loading ? <div className="card flex items-center justify-center gap-2 py-16 text-sm text-ink-500"><Loader2 size={18} className="animate-spin" /> Loading Legacy Access Claims…</div> : claims.length === 0 ? <div className="card py-16 text-center"><Clock3 size={32} className="mx-auto text-ink-300" /><p className="mt-3 font-semibold text-ink-800">No Legacy Access Claims yet</p></div> : (
          <div className="space-y-5">{claims.map((claim) => {
            ...
            return <section key={claim.id} className="overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-sm">
              ...
              <div className="grid gap-5 p-5 lg:grid-cols-2">
                ...
                <div><h3 className="text-sm font-semibold text-ink-800">Submitted evidence</h3><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => openFile(claim.id, 'death-certificate')} className="btn-secondary"><FileText size={15} /> Death certificate <Eye size={14} /></button><button onClick={() => openFile(claim.id, 'identity-proof')} className="btn-secondary"><FileText size={15} /> Identity proof <Eye size={14} /></button>{claim.supportingDocument?.available && <button onClick={() => openFile(claim.id, 'supporting-document')} className="btn-secondary"><FileText size={15} /> Supporting document <Eye size={14} /></button>}</div></div>
              </div>

              <div className="border-t border-ink-100 bg-ink-50/30 px-5 py-5">
                <div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold text-ink-800">Owner-assigned Vault records</h3><p className="mt-1 text-xs text-ink-400">Only these pre-assigned records can be released after Lawyer approval.</p></div><span className="badge bg-brand-50 text-brand-700">{summary.total} total</span></div>
                <div className="mt-4 grid gap-3 sm:grid-cols-4"><SummaryCard Icon={Landmark} label="Assets" value={summary.assets} /><SummaryCard Icon={CreditCard} label="Liabilities" value={summary.liabilities} /><SummaryCard Icon={FolderOpen} label="General" value={summary.general} /><SummaryCard Icon={FileText} label="Total" value={summary.total} /></div>
                {records.length > 0 && <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{records.map((record) => <div key={record.id} className="rounded-xl border border-ink-100 bg-white px-3 py-3"><div className="flex items-center justify-between gap-2"><p className="truncate text-sm font-semibold text-ink-800">{record.title}</p><span className="text-[10px] font-semibold text-brand-700">{record.recordType || 'GENERAL'}</span></div><p className="mt-1 text-xs text-ink-400">{record.category}</p></div>)}</div>}
              </div>

              {['UNDER_ADMIN_REVIEW', 'LEGACY_ACCESS_REQUESTED'].includes(claim.status) && <div className="border-t border-ink-100 bg-white px-5 py-4">{claim.status === 'UNDER_ADMIN_REVIEW' ? <div className="flex flex-wrap justify-end gap-2"><button disabled={workingId === claim.id} onClick={() => review(claim, 'REQUEST_CORRECTION')} className="rounded-lg border border-orange-200 px-3 py-2 text-xs font-semibold text-orange-700 hover:bg-orange-50">Request correction</button><button disabled={workingId === claim.id} onClick={() => review(claim, 'HOLD')} className="rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-600 hover:bg-red-50">Hold</button><button disabled={workingId === claim.id} onClick={() => review(claim, 'REJECT')} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50"><XCircle size={14} className="inline" /> Reject</button><button disabled={workingId === claim.id || summary.total === 0} onClick={() => review(claim, 'FORWARD')} className="btn-primary"><CheckCircle2 size={15} /> Complete platform checks</button></div> : <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end"><select value={selectedLawyers[claim.id] || ''} onChange={(e) => setSelectedLawyers((current) => ({ ...current, [claim.id]: e.target.value }))} className="field-input sm:max-w-sm"><option value="">Select approved Lawyer</option>{lawyers.map((lawyer) => <option key={lawyer.id} value={lawyer.id}>{lawyer.name} · {lawyer.city || 'City not set'} · {lawyer.enrollmentNumber || 'Enrollment unavailable'}</option>)}</select><button disabled={workingId === claim.id} onClick={() => assign(claim)} className="btn-primary"><Scale size={15} /> Assign to Lawyer</button></div>}</div>}
