...
export default function AdminDashboard() {
  ...
  const stats = useMemo(() => {
    const owners = accounts.filter((item) => item.role === 'OWNER').length;
    const beneficiaries = accounts.filter((item) => item.role === 'BENEFICIARY').length;
    const active = accounts.filter((item) => item.status === 'ACTIVE').length;
    return { owners, beneficiaries, active };
  }, [accounts]);

  const filteredAccounts = useMemo(() => {
    const needle = search.trim().toLowerCase();
    return accounts.filter((item) => {
      const matchesSearch = !needle || [item.name, item.username, item.email]
        .filter(Boolean)
        ...
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-ink-500">
              Review registrations, manage platform accounts, and control account access from one dashboard.
            </p>
          </div>
        ...
        <section className="mb-8 overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-sm">
          <div className="flex flex-col gap-4 border-b border-ink-100 px-5 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-ink-400">Search and manage owner and beneficiary access.</p>
            </div>
            <button
              type="button"
              onClick={load}
              className="inline-flex items-center gap-2 self-start rounded-xl border border-ink-100 px-3 py-2 text-sm font-semibold text-ink-600 transition hover:bg-ink-50"
            >
          ...
          {loading ? (
            ...
            <div className="px-5 py-14 text-center text-sm text-ink-400">No accounts match your filters.</div>
          ) : (
                ...
                <tbody className="divide-y divide-ink-100">
                  {filteredAccounts.map((item) => (
                    <tr key={item.id} className="transition hover:bg-ink-50/50">
                      ...
                      <td className="px-5 py-4 text-sm font-medium text-ink-600">{item.role}</td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex justify-end">
                          {item.status === 'ACTIVE' ? (
                            ...
                            >
                              {actionLoadingId === item.id ? <Loader2 size={14} className="animate-spin" /> : <UserRoundX size={14} />}
                              Deactivate
                            </button>
                            ...
                            >
                              {actionLoadingId === item.id ? <Loader2 size={14} className="animate-spin" /> : <UserRoundCheck size={14} />}
                              Activate
                            </button>
                          ) : (
                            <span className="text-xs text-ink-400">Use approval queue</span>
                          )}
                        </div>
                      </td>
                    </tr>
        ...
        <section className="overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-ink-100 px-5 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-ink-400">{pending.length} waiting for identity review</p>
              </div>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="px-5 py-14 text-center">
              ...
              <p className="mt-1 text-sm text-ink-400">There are no pending registrations.</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-ink-100">
