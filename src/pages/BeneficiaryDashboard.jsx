...
function Row({ Icon, label, value }) {
  return (
    <div className="flex items-center gap-3 border-b border-ink-100 py-3 last:border-0">
      <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-ink-50 text-ink-500"><Icon size={16} /></div>
      <div><div className="text-xs font-medium uppercase tracking-wide text-ink-400">{label}</div><div className="text-sm font-semibold text-ink-800">{value}</div></div>
    </div>
  );
}
...
export default function BeneficiaryDashboard() {
  ...
  const loadClaims = useCallback(async () => {
    ...
    try { setClaimError(''); setClaims(await getMyLegacyClaims()); }
    catch (error) { setClaimError(error.message || 'Unable to load Legacy Access Claims.'); }
  ...
  const handleSubmitClaim = async (event) => {
    event.preventDefault(); setClaimError('');
    if (!fileIsValid(deathCertificate) || !fileIsValid(identityProof)) {
      setClaimError('Death certificate and identity proof must be PDF, JPG/JPEG, or PNG files up to 10 MB each.'); return;
    }
    if (supportingDocument && !fileIsValid(supportingDocument)) {
      setClaimError('Supporting document must be PDF, JPG/JPEG, or PNG and no larger than 10 MB.'); return;
    }
    setClaimSubmitting(true);
    try {
      await submitLegacyClaim({ identityProofType, deathCertificate, identityProof, supportingDocument, remarks });
      setDeathCertificate(null); setIdentityProof(null); setSupportingDocument(null); setRemarks(''); await loadClaims();
    } catch (error) { setClaimError(error.message); }
  ...
  const viewDocument = async (documentId) => {
    try {
      ...
      const blob = await response.blob(); const url = URL.createObjectURL(blob); window.open(url, '_blank');
      setTimeout(() => URL.revokeObjectURL(url), 60000);
    } catch (error) { setDocumentError(error.message || 'Unable to access record.'); }
  };
    ...
    <div className="min-h-screen bg-ink-50">
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        <div className="mb-6 flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-brand-50 text-brand-700"><Users size={20} /></div>
          <div><h1 className="text-2xl font-semibold text-ink-900">Beneficiary dashboard</h1><p className="text-sm text-ink-500">Welcome, {user?.name}.</p></div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="card lg:col-span-2">
            ...
            <p className="mt-1 text-sm text-ink-500">Submit this only after the Owner's death. Admin performs platform checks, then an approved Lawyer performs professional review.</p>
            ...
            {claimsLoading ? (
              ...
              <form onSubmit={handleSubmitClaim} className="mt-5 space-y-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <label className="text-sm font-medium text-ink-700">Remarks (optional)<input value={remarks} onChange={(e) => setRemarks(e.target.value)} className="field-input mt-1" placeholder="Anything relevant to the claim" /></label>
                </div>
                <div className="grid gap-4 md:grid-cols-3">
                  {[['Death certificate', deathCertificate, setDeathCertificate, true], ['Identity proof', identityProof, setIdentityProof, true], ['Supporting document', supportingDocument, setSupportingDocument, false]].map(([label, file, setter, required]) => (
                    <label key={label} className="cursor-pointer rounded-xl border border-dashed border-ink-200 bg-white p-4 hover:border-brand-300"><div className="flex items-center gap-2 text-sm font-semibold text-ink-700"><Upload size={15} /> {label}{required ? ' *' : ''}</div><p className="mt-2 truncate text-xs text-ink-500">{file?.name || 'PDF/JPG/PNG up to 10 MB'}</p><input type="file" accept="application/pdf,image/jpeg,image/png" className="sr-only" required={required} onChange={(e) => setter(e.target.files?.[0] || null)} /></label>
                  ))}
                </div>
          ...
          {hasReleasedClaim && (
            <>
              <div className="mt-5 grid gap-3 sm:grid-cols-4">
                <div className="rounded-xl border border-ink-100 p-3"><Landmark size={17} className="text-brand-600" /><p className="mt-2 text-xs text-ink-400">Assets</p><p className="text-xl font-bold text-ink-900">{recordStats.assets}</p></div>
                <div className="rounded-xl border border-ink-100 p-3"><CreditCard size={17} className="text-brand-600" /><p className="mt-2 text-xs text-ink-400">Liabilities</p><p className="text-xl font-bold text-ink-900">{recordStats.liabilities}</p></div>
                <div className="rounded-xl border border-ink-100 p-3"><FileText size={17} className="text-brand-600" /><p className="mt-2 text-xs text-ink-400">General</p><p className="text-xl font-bold text-ink-900">{recordStats.general}</p></div>
                <div className="rounded-xl border border-ink-100 p-3"><FolderOpen size={17} className="text-brand-600" /><p className="mt-2 text-xs text-ink-400">Total</p><p className="text-xl font-bold text-ink-900">{recordStats.total}</p></div>
              </div>
              <div className="mt-4 flex flex-wrap gap-2">{RECORD_TABS.map(([value, label]) => <button key={value} onClick={() => setRecordTab(value)} className={`rounded-xl px-3 py-2 text-xs font-semibold ${recordTab === value ? 'bg-brand-600 text-white' : 'border border-ink-100 bg-white text-ink-600'}`}>{label}</button>)}</div>
            </>
          )}

          {documentError && <div className="alert-error mt-4">{documentError}</div>}
          {documentsLoading ? (
            <div className="flex items-center justify-center gap-2 py-10 text-sm text-ink-500"><Loader2 size={16} className="animate-spin" /> Loading released records...</div>
          ) : visibleDocuments.length === 0 ? (
            <div className="py-10 text-center"><LockKeyhole size={28} className="mx-auto text-ink-300" /><p className="mt-2 text-sm text-ink-500">{hasReleasedClaim ? 'No released records in this section.' : 'Vault information is locked until claim approval.'}</p></div>
          ) : (
            <ul className="mt-4 grid gap-3 md:grid-cols-2">
              {visibleDocuments.map((document) => (
                <li key={document.id} className="rounded-xl border border-ink-100 p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate font-semibold text-ink-800">{document.title}</p><span className="badge bg-brand-50 text-brand-700">{recordTypeLabel(document.recordType)}</span></div><p className="text-sm text-ink-500">{document.category}</p><p className="mt-1 text-xs text-ink-400">From {document.ownerName}{document.ownerUsername ? ` (@${document.ownerUsername})` : ''}</p></div>
                    <button onClick={() => viewDocument(document.id)} className="btn-primary flex shrink-0 items-center gap-2"><Eye size={16} /> View</button>
                  </div>
                </li>
              ))}
            </ul>
